...
def main() -> int:
    ...
    file_path = pathlib.Path(str(payload.get("file_path") or "")).resolve() if payload.get("file_path") else None
    changed_path_managed = bool(file_path and str(file_path) in watch_paths)
    log_event(
        root,
        config,
        "file_changed",
        {
            ...
            "changed_path_managed": changed_path_managed,
            "changed_path": str(file_path) if file_path else None,
        },
    )
    print(hook_json_output(hook_event_name="FileChanged", watch_paths=watch_paths))
    return 0
